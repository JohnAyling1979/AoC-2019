...
const edges1 = getEdges(wire1);
const edges2 = getEdges(wire2);

const interections = [];

for (let i = 0, wire1Steps = 0; i < edges1.length; i++) {
    for (let j = 0, wire2Steps = 0; j < edges2.length; j++) {
        if (edges1[i].type === edges2[j].type) {
            wire2Steps += edges2[j].distance;
            continue;
        }

        if (edges1[i].type === 'h') {
            let lowX = Math.min(edges1[i].point1.x, edges1[i].point2.x);
            let highX = Math.max(edges1[i].point1.x, edges1[i].point2.x)
            let lowY = Math.min(edges2[j].point1.y, edges2[j].point2.y);
            let highY = Math.max(edges2[j].point1.y, edges2[j].point2.y)
            if(
                lowX < edges2[j].point1.x &&
                highX > edges2[j].point1.x &&
                lowY < edges1[i].point1.y &&
                highY > edges1[i].point1.y
            ){
                let partialDistance1;
                if (edges1[i].direction === 'L') {
                    partialDistance1 = highX - edges2[j].point1.x;
                } else {
                    partialDistance1 = edges2[j].point1.x - lowX;
                }

                let partialDistance2;
                if (edges2[i].direction === 'D') {
                    partialDistance2 = highY - edges1[i].point1.y;
                } else {
                    partialDistance2 = edges1[i].point1.y - lowY;
                }

                interections.push(wire1Steps + Math.abs(partialDistance1) + wire2Steps + Math.abs(partialDistance2));
            }
        } else {
            let lowX = Math.min(edges2[j].point1.x, edges2[j].point2.x);
            let highX = Math.max(edges2[j].point1.x, edges2[j].point2.x)
            let lowY = Math.min(edges1[i].point1.y, edges1[i].point2.y);
            let highY = Math.max(edges1[i].point1.y, edges1[i].point2.y)

            if(
                lowX < edges1[i].point1.x &&
                highX > edges1[i].point1.x &&
                lowY < edges2[j].point1.y &&
                highY > edges2[j].point1.y
            ){

                let partialDistance1;
                if (edges1[i].direction === 'D') {
                    partialDistance1 = highY - edges2[j].point1.y;
                } else {
                    partialDistance1 = edges2[j].point1.y - lowY;
                }

                let partialDistance2;
                if (edges2[j].direction === 'L') {
                    partialDistance2 = highX - edges1[i].point1.x;
                } else {
                    partialDistance2 = edges1[i].point1.x - lowX;
                }

                interections.push(wire1Steps + wire2Steps + Math.abs(partialDistance1) + Math.abs(partialDistance2));
            }
        }
        wire2Steps += edges2[j].distance;
    }
    wire1Steps += edges1[i].distance;
}

interections.sort((a, b) => a - b );

console.log('Steps: ', interections[0]);
